...
function movePersonalEntries(targetParent){const selected=state.selected.map(id=>personalFiles.find(x=>x.id===id)).filter(Boolean);selected.forEach(entry=>{const oldPath=[entry.parent,entry.name].filter(Boolean).join('/');const newPath=[targetParent,entry.name].filter(Boolean).join('/');if(entry.type==='folder'){personalFiles.forEach(child=>{if((child.parent||'').startsWith(oldPath+'/'))child.parent=newPath+child.parent.slice(oldPath.length)})}entry.parent=targetParent;entry.modified='2026-06-23 12:08';entry.activity='刚刚 · 移动'})}
