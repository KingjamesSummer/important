/* Department management v11 — final deterministic organization tree. */
(function(){
...
.org-final-head{flex:none;padding:14px;border-bottom:1px solid #e5f0fb;background:#fff}
.org-final-title{display:flex;align-items:center;gap:8px;color:#17446f;font-size:13px;font-weight:750;line-height:1}
.org-final-title>.icon-stack{width:17px;height:17px;color:#1769ff}.org-final-title small{margin-left:auto;color:#6284a5;font-size:8px;font-weight:550}
...
`;
  ...
  function allSearchText(node){
    return [node.name,...(node.children||[]).map(allSearchText)].flat().join(' ').toLowerCase();
  }

  function menuHtml(node){
    if(!window.state||state.adminDeptTreeMenu!==node.id)return '';
    return `<div class="org-final-menu" onclick="event.stopPropagation()"><button onclick="openAdminDeptDialog('create','${node.id}')">${iconHtml('plus')}新建下级部门</button><button onclick="selectAdminDept('${node.id}');openAdminDeptDialog('edit')">${iconHtml('edit')}编辑部门</button><button onclick="selectAdminDept('${node.id}');openAdminDeptDialog('move')">${iconHtml('move')}调整上级部门</button><button class="danger" onclick="selectAdminDept('${node.id}');openAdminDeptDialog('decommission')">${iconHtml('trash')}组织裁撤</button></div>`;
  }

  function nodeHtml(node,depth=0){
    const children=node.children||[];
    const expanded=window.state&&Array.isArray(state.adminDeptExpanded)&&state.adminDeptExpanded.includes(node.id);
    const selected=window.state&&state.adminDeptSelected===node.id;
    return `<div class="org-final-node" data-search="${escapeHtml(allSearchText(node))}"><div class="org-final-row ${depth===0?'root':''} ${selected?'active':''}"><button class="org-final-toggle ${children.length?'':'empty'} ${expanded?'':'collapsed'}" onclick="event.stopPropagation();toggleAdminDept('${node.id}')">${children.length?iconHtml('down'):''}</button><button class="org-final-select" onclick="selectAdminDept('${node.id}')">${iconHtml(node.type==='company'?'building':'folder')}<span class="org-final-name">${escapeHtml(node.name)}</span></button><button class="org-final-more" onclick="event.stopPropagation();toggleAdminDeptTreeMenu('${node.id}')">${iconHtml('more')}</button>${menuHtml(node)}</div>${children.length?`<div class="org-final-children ${expanded?'':'collapsed'}">${children.map(child=>nodeHtml(child,depth+1)).join('')}</div>`:''}</div>`;
  }

  function panelHtml(){
    const selected=window.state&&state.adminDeptSelected==='unassigned';
    return `<div class="org-final-head"><div class="org-final-title">${iconHtml('building')}组织架构<small>集团 / 部门 / 项目组</small></div><div class="org-final-tools"><label class="org-final-search">${iconHtml('search')}<input id="orgFinalSearch" placeholder="搜索部门名称" oninput="filterFinalOrgTree(this.value)"></label><button class="org-final-expand" title="展开或收起全部" onclick="toggleAdminDeptAll()">${iconHtml('down')}</button></div></div><div class="org-final-scroll" id="orgFinalScroll"><div class="org-final-section">正式组织</div>${nodes.map(item=>nodeHtml(item)).join('')}<div class="org-final-section">特殊节点</div><div class="org-final-node" data-search="待分配人员池"><div class="org-final-row ${selected?'active':''}"><span class="org-final-toggle empty"></span><button class="org-final-select" onclick="selectAdminDept('unassigned')">${iconHtml('users')}<span class="org-final-name">待分配人员池</span></button><span class="org-final-more"></span></div></div><div class="org-final-empty" id="orgFinalEmpty" hidden>未找到匹配的组织节点</div></div>`;
  }
  ...
  function rebuild(){
    if(!window.state||state.page!=='admin'||state.adminTab!=='dept')return;
